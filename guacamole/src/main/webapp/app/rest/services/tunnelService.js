...
angular.module('rest').factory('tunnelService', ['$injector',
    function tunnelService($injector) {
        ...
        service.downloadStream = function downloadStream(tunnel, stream, mimetype, filename) {
            ...
            var is_print_pdf = false;
            if (mimetype === "application/pdf" && (isChrome || isOpera || isFirefox)) {
                is_print_pdf = true;
                url = url + '&type=' + encodeURIComponent(mimetype);
            }

            if (isFirefox && is_print_pdf) {                
                var viewer_url = 'pdfjs/web/viewer.html?file=' + encodeURIComponent(url) + '&print=';
            ...
            if (is_print_pdf) iframe.setAttribute("id", "print_iframe");
            ...
            var downloadComplete = function downloadComplete() {
                ...
                $window.setTimeout(function cleanupIframe() {
                    if (!is_print_pdf) {
                        ...
                        iframe.interval = setInterval(function () {
